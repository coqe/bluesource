...
import { getSearchEntities, getEntities } from './user-profile.reducer';
      ...
      <div>
        <h2 id="user-profile-heading">
          ...
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp; Create new User Profile
          </Link>
        </h2>
        <Row>
          <Col sm="12">
              ...
              {userProfileList.map((userProfile, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    {userProfile.avatar ? (
                      <div>
                        <a onClick={openFile(userProfile.avatarContentType, userProfile.avatar)}>
                          <img src={`data:${userProfile.avatarContentType};base64,${userProfile.avatar}`} style={{ maxHeight: '30px' }} />
                          &nbsp;
                        </a>
                        <span>
                          {userProfile.avatarContentType}, {byteSize(userProfile.avatar)}
                        </span>
                      </div>
                    ) : null}
                  </td>
                  <td>{userProfile.account ? userProfile.account.login : ''}</td>
                  <td>
                    {userProfile.skills
