...
export const ACTION_TYPES = {
  SEARCH_SKILLS: 'skill/SEARCH_SKILLS',
...
export default (state: SkillState = initialState, action): SkillState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_SKILLS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_SKILLS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_SKILLS):
      return {
        ...state,
        loading: false,
...
export const getSearchEntities: ICrudSearchAction<ISkill> = query => ({
  type: ACTION_TYPES.SEARCH_SKILLS,
  payload: axios.get<ISkill>(`${apiSearchUrl}?query=` + query)
});
